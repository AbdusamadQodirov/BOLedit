...
def guess_state_code_from_text(text: str) -> Optional[str]:
    ...
    # kelishi shart (masalan "SPRINGDALE AR 72764", "Mt. Juliet, TN 37122").
    # Bu pattern ENG ISHONCHLI bo'lgani uchun BIRINCHI tekshiriladi - OCR
    # shovqinida vergul-pattern ko'pincha noto'g'ri/yo'q moslik beradi, zip
    # ketidan kelgan shtat kodi esa deyarli har doim to'g'ri.
    m = re.search(r'\b([A-Z]{2})\s+\d{5}(-\d{4})?\b', section)
    if m and m.group(1) in _STATE_TO_IANA:
        return m.group(1)

    # "(MN)" yoki ", MN" yoki ",MN" kabi (zip topilmasa, fallback sifatida)
