...
def _image_to_base64(img: Image.Image) -> tuple:
    ...
    Anthropic API limiti: max 5MB, max 8000px.
    Kichraytirish + JPEG siqish bilan xavfsiz o'lchamga keltiramiz.
    """
    MAX_DIM = 1568  # Anthropic tavsiya qiladigan max o'lcham
    MAX_BYTES = 4 * 1024 * 1024  # 4MB limit (5MB dan xavfsiz past)
    ...
    # RGB ga o'tkazish (PNG da alpha kanal bo'lsa JPEG saqlay olmaydi)
    ...
    # Sifatni kamaytira borib 4MB dan kichik bo'lguncha siqamiz
    quality = 90
    while quality >= 50:
        buf = io.BytesIO()
...
def find_vision_candidates(img: Image.Image, model: str = "claude-sonnet-4-6") -> List[VisionCandidate]:
    ...
    """
    client = _get_client()
    img_b64, media_type = _image_to_base64(img)
