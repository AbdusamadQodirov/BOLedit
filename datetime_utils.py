...
class TimeMatch:
    ...
    is_range: bool = False     # "06:00-10:00" kabi vaqt oralig'imi
    range_end_hour: Optional[int] = None    # oraliq bo'lsa, tugash soati
    range_end_minute: Optional[int] = None  # oraliq bo'lsa, tugash daqiqasi
...
_PATTERNS = [
    # 0) MM/DD/YYYY HH:MM-HH:MM (vaqt ORALIG'I, masalan Armstrong BOL "6/19/2026 06:00-10:00 CST")
    #    Eng yuqori ustuvorlikda - aks holda mdy_slash uni noto'g'ri ikkiga bo'lib oladi.
    (re.compile(
    ...
    for pattern, date_style, _ in _PATTERNS:
        for m in pattern.finditer(text):
            ...
            if is_range:
                ...
                # range holatida asl pattern "ampm1" (boshlanish vaqti yonidagi,
                # odatda bo'sh) va "ampm" (tugash vaqti yonidagi, asosiy) bor -
                # has_ampm ni "ampm" asosida to'g'irlaymiz (allaqachon yuqorida hisoblangan)
...
def format_like_original(dt: datetime, tm: TimeMatch) -> str:
    ...
    if tm.is_range and tm.range_end_hour is not None:
        # Vaqt ORALIG'I: masalan "6/19/2026 06:00-10:00 CST".
        # dt - oraliqning YANGI BOSHLANISH vaqti. Asl oraliq DAVOMIYLIGINI
        # (masalan 4 soat) saqlab, yangi tugash vaqtini shunga qarab hisoblaymiz.
        old_start_match = re.search(r'(\d{1,2}):(\d{2})', tm.raw_text)
        if old_start_match:
            old_start_h, old_start_m = int(old_start_match.group(1)), int(old_start_match.group(2))
        ...
        date_part = build_date_str()
        start_part = f"{dt.hour:02d}:{dt.minute:02d}"
        end_part = f"{new_end_h:02d}:{new_end_m:02d}"
